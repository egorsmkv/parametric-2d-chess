...
def _stack(widths: list[float], y0: float) -> tuple[Sketch, float]:
...
def make_pawn(scale: float = 1.0) -> Sketch:
    ...
    body, top = _stack([8.6, 7.4, 6.2], 5.2)
...
def make_rook(scale: float = 1.0) -> Sketch:
    ...
    body, top = _stack([9.6, 8.6, 7.4, 6.2], 5.2)
...
def make_knight(scale: float = 1.0, facing: str = "left") -> Sketch:
    ...
    body, top = _stack([9.6, 8.2, 6.8], 5.2)
    head = [
        (5.2, top),
        (6.2, top + 6.0),
        (4.6, top + 10.6),
...
def make_bishop(scale: float = 1.0) -> Sketch:
    ...
    body, top = _stack([8.6, 7.6, 6.6, 5.6], 5.2)
...
def make_queen(scale: float = 1.0) -> Sketch:
    ...
    body, top = _stack([9.8, 8.8, 7.8, 6.8, 5.8], 5.2)
    tips = [
        (-7.0, top + 4.2),
        (-3.6, top + 6.6),
        (0.0, top + 8.2),
...
def make_king(scale: float = 1.0) -> Sketch:
    ...
    body, top = _stack([10.2, 9.2, 8.2, 7.2, 6.2, 5.2], 5.2)
    tips = [
        (-6.4, top + 3.8),
        (-3.2, top + 6.0),
        (0.0, top + 7.4),
